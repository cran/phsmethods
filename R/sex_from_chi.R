...
#'
#' @description `sex_from_chi` takes a CHI number or a vector of CHI numbers
...
#' and `'Female'`. Note that this will override any custom values supplied with
#' `male_value` or `female_value`.
...
#' CHI(s).
#'
#' @return a vector with the same class as `male_value` and `female_value`,
...
sex_from_chi <- function(chi_number,
  ...
  if (as_factor) {
    sex <- factor(
      x = sex,
      levels = c(male_value, female_value),
